...
Launch Template (Scalable-App-LT): Defines exactly what an EC2 instance should look like, leveraging a dynamically fetched Amazon Linux 2 AMI and a User Data script that installs an Apache HTTP server serving a dynamic HTML page displaying the Instance ID.
Application Load Balancer (Scalable-Web-ALB): Spans across two Availability Zones in the default VPC, listening for traffic on port 80.
Auto Scaling Group (Scalable-App-ASG): Dictates a minimum of 2 instances and a max of 4. Automatically places the spun-up instances into the Application Load Balancer's Target Group.
Start the Project
